...

{
  ...
  lib,
  nix,
  nix-daemon-tunnel-socket ? "/var/lib/nix-daemon-tunnel/socket",
  runCommand,
}:
...
with rec {
  ...
  warn =
    with rec {
      ...
      WITH_NIX_WARNING =
        if pathExists nix-daemon-tunnel-socket then
          warning false
        else
          trace (warning true) warning false;
    };
  ...
  vars = remote // {
    NIX_PATH =
      if getEnv "NIX_PATH" == "" then "nixpkgs=${<nixpkgs>}" else getEnv "NIX_PATH";
  };
};
attrs:
vars
