...
with rec {
  ...
  inDir = runCommand "${name}-unwrapped" {
    f = writeScript "${name}-raw"
      (if patchShebangs then patchShebang { string = script; } else script);
  } ''
    mkdir "$out"
  ...
  f = if file == null then
    newFile
  else if patchShebangs then
    patchShebang { inherit file name; }
  else
    file;

  # Whether any extra env vars or paths are actually needed
  needEnv = if paths == [ ] && vars == { } then "false" else "true";
...
runCommand name (pathData.env // varNameData.env // varValData.env // {
  inherit f needEnv;
