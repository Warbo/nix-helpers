{ cabal-install, callPackage, ghc, hackageIndex, lib, runCommand }:
with { oldHackageIndex = hackageIndex; };
{ name, cabalFile, doCheck ? true, doBench ? false
, hackageIndex ? oldHackageIndex }:
with {
  plan = runCommand "${name}-plan.json" {
    buildInputs = [
      cabal-install
      ghc
    ...
    cabal update
    cp ${lib.escapeShellArg "${cabalFile}"} ./${
      lib.escapeShellArg "${name}.cabal"
    }
